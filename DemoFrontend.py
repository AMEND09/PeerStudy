...
class NoteSharingApp:
    def __init__(self, page: ft.Page):
        ...
        self._demo_data = {
            "groups": [
                {'id': 101, 'name': 'Calculus Crew', 'course_code': 'MATH-101', 'member_count': 4, 'join_code': 'CALC-ABC'},
                {'id': 102, 'name': 'Physics Phantoms', 'course_code': 'PHY-203', 'member_count': 2, 'join_code': 'PHYS-XYZ'}
            ],
            101: {
                "notes": [
                    {'title': 'Chapter 1: Limits', 'content': 'Remember to review L\'Hôpital\'s rule.'},
                    {'title': 'Useful Link for Derivatives', 'content': 'https://www.mathsisfun.com/calculus/derivatives-rules.html'}
                ],
                "meetups": [
                    {'topic': 'Midterm Review Session', 'time': datetime.now(timezone.utc).isoformat(), 'description': 'Covering chapters 1-4. Bring questions!', 'link': 'https://zoom.us/j/1234567890'},
                    {'topic': 'Problem Set 5 Walkthrough', 'time': '2024-12-27T19:30:00Z', 'description': 'We will work through the tough questions together.', 'link': ''}
                ],
                "chat": [
                    {'author': 'Alice', 'text': 'Hey everyone, ready for the midterm?'},
                    {'author': self.test_user['username'], 'text': 'I am! Just need to review chapter 3.'},
                    {'author': 'Bob', 'text': 'Same here. That section on related rates is tricky.'}
                ]
            }
        }
    ...
    def api_call(self, method, endpoint, data=None):
        if self.is_demo_mode:
            ...
            if method == 'GET' and endpoint == '/groups':
                return self._demo_data["groups"], None
            if method == 'GET' and endpoint == f'/groups/{group_id}':
                 return next((g for g in self._demo_data["groups"] if g['id'] == group_id), None), None
            if method == 'GET' and endpoint.endswith('/notes'):
                return self._demo_data.get(group_id, {}).get("notes", []), None
            if method == 'GET' and endpoint.endswith('/meetups'):
    ...
    def populate_meetups_list(self, meetups_data):
        ...
        if meetups_data:
            for m in meetups_data:
                parsed_time = datetime.fromisoformat(m['time'].replace('Z', '+00:00'))
                local_time_str = parsed_time.astimezone().strftime('%A, %b %d @ %I:%M %p %Z')
                self.meetups_list.controls.append(ft.Card(ft.ListTile(leading=ft.Icon(ft.Icons.CALENDAR_MONTH), title=ft.Text(m['topic'], weight=ft.FontWeight.BOLD), subtitle=ft.Text(f"{local_time_str}\n{m['description']}"), trailing=ft.IconButton(ft.Icons.LINK, url=m['link'], disabled=not m['link'], tooltip="Join Meeting"))))
    ...
    def get_join_group_view(self):
        ...
        def join_click(e):
            ...
            if result and not error: 
                self.show_success_snackbar(result.get("message", "Successfully joined group!"))
                self.page.go("/dashboard")
            else: self.show_error_dialog(error)
    ...
    def get_login_view(self):
        username_field = ft.TextField(label="Username", autofocus=True, value=self.test_user["username"]);
        password_field = ft.TextField(label="Password", password=True, on_submit=lambda e: login_click(e), value=self.test_user["password"])
        
        def login_click(e):
            uname = username_field.value
            pword = password_field.value
            ...
            if uname == self.test_user["username"] and pword == self.test_user["password"]:
                ...
                self.page.client_storage.set("auth_token", "DEMO_TOKEN")
                self.page.client_storage.set("user_id", self.test_user['user_id'])
                self.page.client_storage.set("username", self.test_user['username'])
                self.show_success_snackbar("Demo login successful!")
                self.page.go("/dashboard")
                return
            ...
            if result and 'user_id' in result:
                ...
                self.page.client_storage.set("user_id", result['user_id'])
                self.page.client_storage.set("username", uname)
        ...
        return ft.View("/login", [
            ft.Column([
                ...
                ft.Text("Enter testuser/password123 for a demo.", italic=True, size=12),
